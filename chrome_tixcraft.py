...
def tixcraft_area_auto_select(driver, url, config_dict):
    ...
    try:
        ...
        # 當找不到區域選擇框 (.zone) 時，檢查是否為阻斷頁面
        page_source = driver.page_source
        detection_keywords = ["Browsing Activity Has Been Paused", "限制存取", "unusual behavior"]
        
        if any(keyword in page_source for keyword in detection_keywords):
            print(f"[{datetime.now().strftime('%H:%M:%S')}] 偵測到阻斷頁面，執行回上一頁...")
            try:
                driver.back() # 僅執行回上一頁
                time.sleep(random.uniform(1.0, 5.0)) # 給予一點緩衝時間避免循環過快
            except Exception:
                pass
            return # 結束本次執行，讓主迴圈重新嘗試
