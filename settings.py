...
# --- 新增的部分：清除 UC 的系統快取資料夾 ---
    # 使用 os.path.join 結合環境變數，確保程式在不同電腦也能運作
    uc_cache_path = os.path.join(os.environ.get('APPDATA', ''), 'undetected_chromedriver')
    
    if os.path.exists(uc_cache_path):
        try:
            # 強制刪除整個資料夾及其內容
            shutil.rmtree(uc_cache_path, ignore_errors=True)
            print(f"成功清理 UC 快取資料夾: {uc_cache_path}")
        except Exception as exc:
            print(f"清理 UC 快取失敗: {exc}")
